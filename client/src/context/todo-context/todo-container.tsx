'use client'
import React, { ComponentType, FC, PropsWithChildren, useLayoutEffect, useState } from 'react'
...
const TodoContainer: FC<PropsWithChildren> = ({ children }) => {
  ...
  const addTodo = async (name: string, description: string, callback: () => void) => {
    try {
  ...
  const updateTodo = async (id: string, updates: Partial<Todo>, callback: () => void) => {
    try {
      const { data, error } = await TodoService.updateTodo(id, updates)
      if (data) {
        setTodoState((prev) => ({
          ...prev,
          todoList: prev.todoList.map((todo) => (todo._id === id ? { ...todo, ...updates } : todo)),
        }))
      }
    } catch (error) {
      console.log('error value is:', error)
...
export const withTodoContainer = <P extends object>(WrappedComponent: ComponentType<P>): FC<P> => {
